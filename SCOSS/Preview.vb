...
Public Class Preview
    ...
    Dim curr As String
    Dim roundad As Decimal = 0
    Dim disc As Decimal = 0
    Dim suppsst, suppbrn As String
    ...
    Private Sub Preview_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Else
            textboxPB.Text = table.Rows(0)(0).ToString
            textboxRB.Text = table.Rows(0)(1).ToString
            textboxCB.Text = table.Rows(0)(2).ToString
            textboxAB.Text = table.Rows(0)(3).ToString
        End If
    ...
    Private Sub btnGenerate_Click(sender As Object, e As EventArgs) Handles btnGenerate.Click
        ...
        conn.Open()
        Dim cmd As New SqlCommand("SELECT [SCO_NO],[SCO_CDATE],[SCO_SUPP],[SCO_CURR],[SCO_PURP],[SCO_RE],[SCO_STOCK],[SCO_ETA] ,[SCO_SECTION],[SCO_MACHNO],[SCO_PART],[SCO_INV],[SCO_UOM],[SCO_QTY],[SCO_UPRICE],[SCO_AMOUNT],[SCO_DISC],[SCO_TAX],[SCO_TOTAL],[SCO_CO],[SCO_PRINT],[SCO_ROUND] FROM SCO_TEMP WHERE SCO_NO = @no ORDER BY SCO_STOCK", conn)
        ...
        ElseIf Sizecb.SelectedIndex = 1 Then
            conn.Open()
            Dim commands As New SqlCommand("SELECT * FROM SCO_CO", conn)
            Dim adptr As New SqlDataAdapter(commands)
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                conn.Open()
                Dim cmd2 As New SqlCommand("SELECT SUPP_DESC, SUPP_ADDRESS, SUPP_FAX, SUPP_TEL, SUPP_SSTNO, SUPP_BRN FROM SCO_SUPP WHERE SUPP_CODE = @code ", conn)
                ...
                suppsst = table2.Rows(0)(4).ToString
                suppbrn = table2.Rows(0)(5).ToString
            ...
            objrpt.SetParameterValue("gst", tbl.Rows(0)(6).ToString)
            objrpt.SetParameterValue("header", tbl.Rows(0)(3).ToString)
            objrpt.SetParameterValue("company", tbl.Rows(0)(1).ToString)
            objrpt.SetParameterValue("tel", tbl.Rows(0)(4).ToString)
            objrpt.SetParameterValue("fax", tbl.Rows(0)(5).ToString)
            objrpt.SetParameterValue("cocode", tbl.Rows(0)(2).ToString)
            objrpt.SetParameterValue("prepared", textboxPB.Text)
            objrpt.SetParameterValue("confirmed", textboxRB.Text)
            objrpt.SetParameterValue("authorised", textboxCB.Text)
            objrpt.SetParameterValue("gstno", suppsst)
            objrpt.SetParameterValue("brn", suppbrn)
            ' objrpt.SetParameterValue("Copies", "Purchasing Copy")
            ...
            objrpt.SetParameterValue("roundAdj", "")
            ...
            ' curr = dt.Rows(0)(3).ToString
        ElseIf Sizecb.SelectedIndex = 2 Then
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                ' adjust amount
                newdata("Amount") = Val(dt.Rows(i)(15).ToString) + Val(dt.Rows(i)(17).ToString)

                newdata("Discount") = 0
                newdata("Tax") = Val(dt.Rows(i)(17).ToString)
                newdata("Total") = Val(dt.Rows(i)(15).ToString)
                newdata("Address") = suppadd
                newdata("Tel") = "Tel: " & supptel
                newdata("Fax") = "Fax: " & suppfax
                ...
                disc += Val(dt.Rows(i)(16).ToString)
                roundad += Val(dt.Rows(i)(21).ToString)
            Next

            Dim PkSize As New System.Drawing.Printing.PaperSize("9.5x12", 9.5, 12)

            ' 9.5" x 12" size
            ' this part probably is the main problem for the constant 'load report failed' error
            ...
            'objrpt.SetParameterValue("company", MainMenu.ToolStripStatusLabel3.Text)
            objrpt.SetParameterValue("Currency", dt.Rows(0)(3).ToString)
            objrpt.SetParameterValue("roundAdj", roundad)
            objrpt.SetParameterValue("disc", disc)
            'objrpt.SetParameterValue("roundAdj", roundad)
            CrystalReportViewer1.ReportSource = objrpt
            CrystalReportViewer1.Refresh()
            ...
            curr = dt.Rows(0)(3).ToString
        Else
            MsgBox("Please choose the paper size.", MsgBoxStyle.OkOnly, "Warning")
        End If

    End Sub

    Private Sub Preview_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        TextBox1.Text = ""
        Sizecb.SelectionStart = 0
        roundad = 0
        disc = 0
    ...
    Sub printBWA4()
        ...
        Dim PrintDialog As New PrintDialog()
        Dim objrpt As New PreviewSP
        Dim pb As String = textboxPB.Text & " Copy"
        Dim rb As String = textboxRB.Text & " Copy"
        Dim cb As String = textboxCB.Text & " Copy"

        Dim copies() As String = {pb, rb, cb}
        'objectreport.PrintOut(False, 3)
